...
export function ResetPasswordForm() {
  const [state, action, pending] = useActionState<ResetPasswordState, FormData>(resetPasswordAction, null);
  return <form action={action} className="mt-6 space-y-4">
    ...
    <button className="rounded-md bg-lp-900 px-4 py-2 font-medium text-white disabled:opacity-60" disabled={pending} type="submit">{pending ? "Saving…" : "Reset password and continue"}</button>
  </form>;
}
