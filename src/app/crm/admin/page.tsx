...
function AdministrationCardLink({ card }: { card: AdministrationCard }) {
  return (
    ...
    >
      <h2 className="font-semibold text-slate-950 group-hover:text-slate-700">{card.title}</h2>
      <p className="mt-2 text-sm leading-6 text-slate-600">{card.description}</p>
      <span className="mt-4 inline-block text-sm font-medium text-slate-900">Open →</span>
    </Link>
  );
}
  ...
  return (
    <section className="space-y-6">
      <div>
        ...
        <h1 className="mt-1 text-2xl font-semibold text-slate-950 sm:text-3xl">CRM administration</h1>
