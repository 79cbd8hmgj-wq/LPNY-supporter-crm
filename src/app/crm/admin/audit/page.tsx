    ...
    <div className="space-y-5">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-semibold tracking-tight text-slate-950">Audit log</h1>
      ...
      <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-200 px-4 py-3">
          <h2 className="font-semibold text-slate-950">Administrative events</h2>
        ...
        {audit.events.length === 0 ? (
          <div className="px-4 py-12 text-center">
            <p className="font-medium text-slate-800">No audit events are available on this page.</p>
          </div>
        ) : (
              ...
              return (
                <article key={event.id} className="grid gap-3 px-4 py-4 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)_auto] lg:items-start">
                  <div className="min-w-0">
                    <h3 className="font-semibold text-slate-950">{formatAuditAction(event.action_type)}</h3>
                    <p className="mt-1 text-sm text-slate-600">By {event.actorDisplayName}</p>
                    {metadata.length > 0 ? (
                      <dl className="mt-3 flex flex-wrap gap-2">
                        {metadata.map((item) => (
                          <div key={`${item.label}-${item.value}`} className="rounded-lg bg-slate-50 px-2.5 py-1.5 text-xs text-slate-700">
                  ...
                  <div className="text-sm text-slate-600">
                    ...
                    {event.target_id ? (
                      <div className="mt-1 break-all font-mono text-xs text-slate-500">{event.target_id}</div>
                    ) : (
                      <div className="mt-1 text-xs text-slate-500">No single record target</div>
                    )}
                  </div>

                  <time className="text-xs text-slate-500 lg:text-right" dateTime={event.occurred_at}>
        ...
        {audit.totalPages > 1 ? (
          <nav className="flex items-center justify-between gap-3 border-t border-slate-200 px-4 py-3" aria-label="Audit log pagination">
            {audit.page > 1 ? (
              <Link
