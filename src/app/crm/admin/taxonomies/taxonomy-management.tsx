...
function StatusBadge({ active }: { active: boolean }) {
  return (
    <span className={`w-fit rounded-full px-2 py-1 text-xs font-medium ${active ? "bg-emerald-50 text-emerald-700" : "bg-lp-100 text-lp-600"}`}>
...
function ActiveToggle({ active, onChange, disabled }: { active: boolean; onChange: (active: boolean) => void; disabled?: boolean }) {
  return (
    <label className="flex min-h-11 items-center gap-2 rounded-lg border border-lp-200 bg-lp-50 px-3 text-sm font-medium text-lp-800">
...
function InterestCreateForm() {
  ...
  return (
    <form className="grid gap-3 rounded-lg border border-lp-200 bg-lp-50 p-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end" onSubmit={submit}>
...
function InterestEditor({ record }: { record: InterestAdminRecord }) {
  ...
  return (
    <article className="space-y-3 rounded-lg border border-lp-200 p-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <code className="text-xs text-lp-500">{record.slug}</code>
      ...
      <p className="text-xs leading-5 text-lp-500">The slug above is permanent and will not change when this name is edited.</p>
...
function SourceCreateForm() {
  ...
  return (
    <form className="grid gap-3 rounded-lg border border-lp-200 bg-lp-50 p-3 md:grid-cols-[minmax(0,1fr)_minmax(11rem,0.5fr)_auto] md:items-end" onSubmit={submit}>
      <label className="text-sm font-medium text-lp-800">
        New source
...
function SourceEditor({ record }: { record: SourceAdminRecord }) {
  ...
  return (
    <article className="space-y-3 rounded-lg border border-lp-200 p-3">
      ...
      <ActiveToggle active={active} disabled={pending} onChange={setActive} />
      <p className="text-xs leading-5 text-lp-500">The slug above is permanent. Category and display name may be updated without breaking historical source links.</p>
      <ResultMessage result={result} />
      <button className="min-h-10 rounded-lg border border-lp-300 px-3 py-2 text-sm font-semibold text-lp-800 hover:bg-lp-50 disabled:cursor-not-allowed disabled:opacity-60" disabled={pending} onClick={save} type="button">
...
function SectionHeader({ title, description, count }: { title: string; description: string; count: number }) {
  return (
    <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between">
      <div>
        <h2 className="text-lg font-semibold text-lp-950">{title}</h2>
        <p className="mt-1 text-sm leading-6 text-lp-600">{description}</p>
      </div>
      <span className="text-sm text-lp-500">{count.toLocaleString()} total</span>
    </div>
  );
}
...
export function TaxonomyManagement({
  ...
  return (
    <fieldset className="space-y-6 border-0 p-0" disabled={!hydrated} aria-busy={!hydrated}>
      <section className="space-y-4 rounded-xl border border-lp-200 bg-white p-4 shadow-sm sm:p-5">
      ...
      <section className="space-y-4 rounded-xl border border-lp-200 bg-white p-4 shadow-sm sm:p-5">
        ...
        <TagCreateForm />
        {tags.length === 0 ? (
          <p className="rounded-lg border border-dashed border-lp-300 p-6 text-center text-sm text-lp-500">No tags have been created yet.</p>
        ) : (
          <div className="grid gap-3 xl:grid-cols-2">
