...
export default async function WorkPage() {
  ...
  const [{ data: people }, { data: events }] = await Promise.all([
    ...
    supabase.from("crm_events").select("id, title, location, starts_at, ends_at, visibility").gte("starts_at", new Date().toISOString()).order("starts_at").limit(20),
  ]);
