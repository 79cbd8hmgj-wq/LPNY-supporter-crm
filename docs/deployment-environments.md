...
Both required values belong only in the protected GitHub `staging` environment. The Supabase service-role key authorizes disposable staging fixtures; the Vercel automation bypass authorizes CI through Vercel Deployment Protection without making the preview public. Do not copy either value into repository files, workflow inputs, logs, or chat.
