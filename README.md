...
The hosted workflow versions the public Staging app URL, Supabase URL, and publishable browser key. The protected GitHub `staging` environment stores `STAGING_SUPABASE_SERVICE_ROLE_KEY` for staging test fixtures and `STAGING_VERCEL_AUTOMATION_BYPASS_SECRET` so CI can pass Vercel Deployment Protection without making the preview public. Do not run the full mutating E2E suite against Production.

For repeatable hosted acceptance, point the dedicated `staging` branch at the exact verified `main` release commit. That branch push automatically starts `Staging E2E`; `workflow_dispatch` remains available for reruns. The workflow reads only the two protected staging credentials from the GitHub `staging` environment and verifies both the exact deployed commit and the app's Staging data target before running Chromium/WebKit.
